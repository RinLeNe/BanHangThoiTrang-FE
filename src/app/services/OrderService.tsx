...
export const orderById = async (editId: any) => {
  const response = await axiosClient.get(
    `http://localhost:8080/api/orders/${editId}`
  );
  return response;
};
...
export const deleteOrder = async (deleteId: any) => {
  const response = await axiosClient.delete(
    `http://localhost:8080/api/orders/${deleteId}`
  );
  return response;
};
export const updateOrder = async (updId: any, orderDetail: orderPayload) => {
  const response = await axiosClient.put(
    `http://localhost:8080/api/orders/${updId}`,
    JSON.stringify(orderDetail)
  );
  return response;
};
export const cancelOrder = async (updId: any) => {
  const response = await axiosClient.put(
    `http://localhost:8080/api/orders/cancelOrder/${updId}`  );
  return response;
};
export const repurchaseOrder = async (orderId:any) => {
  const response = await axiosClient.post(`http://localhost:8080/api/orders/repurchase/${orderId}`);
  return response;
}